...
def QuantumEnergy(N, board_stack, A, B, C, J_perp):
    ...
    # print("Coupling energy:", coupling_energy)
    ...
    #
    # total_energy = E_classical + J_perp * E_coupling # is the coupling term too strong?
    # return total_energy
...
def RunPIQA(N, P, T, monte_carlo_steps, annealing_steps, initial_Gamma, final_Gamma, A, B, C):
    ...
    for step in range(annealing_steps):
        ...
        J_perpendicular = -1/2 * (P*T) * np.log(np.tanh(Gamma / (P*T)))
        print("J_perpendicular:", J_perpendicular)
...
def RunMultipleQuantumAnnealing(num_runs, N, P, T, monte_carlo_steps, annealing_steps,
                                  initial_Gamma, final_Gamma, A, B, C):
    all_best_energies = []
    # total_steps is the total number of MC moves per run
    total_steps = monte_carlo_steps * annealing_steps
    best_overall_energy = float("inf")
    best_overall_board = None

    for i in range(num_runs):
        # Call RunPIQA with all required arguments in the correct order.
        energies, final_energy, final_board = RunPIQA(N, P, T, monte_carlo_steps, annealing_steps,
                                                       initial_Gamma, final_Gamma, A, B, C)

        # Keep track of the best (lowest energy) final board among runs.
    ...
    # Average best energy over runs (at each MC move index)
...
# Call the multiple-run wrapper
average_best_energy, final_board, final_energy = RunMultipleQuantumAnnealing(
    num_runs, N, P, T, monte_carlo_steps, annealing_steps, initial_Gamma, final_Gamma, A, B, C)

print("Average best energy:", average_best_energy)
total_steps = monte_carlo_steps * annealing_steps

# For plotting, sample 15 evenly spaced points over the total steps.
...
print("Final Board:")
PrintReadableBoard(final_board)
...
# *** Uncomment below if doing just 1 run of Path Integral Quantum Annealing ***
